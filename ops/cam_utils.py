...
class CamPlanner():
    ...
    def rotate_right(self, angle, num_frames=30):
        """Rotate camera to the right (yaw) by `angle` degrees."""
        self.move_with_rotate(0, angle, rotate="right", num_frames=num_frames)
        return self

    def rotate(self, angle, num_frames=30):
        """
        Backward-compatible alias.
        Positive angle => rotate_left; negative angle => rotate_right.
        """
        if angle >= 0:
            return self.rotate_left(angle, num_frames=num_frames)
        return self.rotate_right(-angle, num_frames=num_frames)
